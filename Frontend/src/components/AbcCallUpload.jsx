...
const AbcCallUpload = () => {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      // Changed endpoint to abc-calls
      const response = await fetch(`${API_BASE}/api/abc-calls/upload`, {
        method: 'POST',
        body: formData
      });
  ...
  return (
    <div className="min-h-screen bg-[#08080c] text-gray-100">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-3xl font-semibold tracking-tight" style={{ fontFamily: "'Fraunces', serif", letterSpacing: '-0.02em' }}>
                Upload ABC Cart Recovery Calls
              </h1>
              <p className="text-gray-400 text-sm mt-1">Upload a CSV with audio recordings for automated cart recovery analysis</p>
            </div>
          </div>
        </div>
      </header>
      ...
      <main className="max-w-4xl mx-auto px-6 py-12">
        ...
        {/* Instructions */}
        <div className="bg-[#0f0f14] border border-red-500/20 rounded-2xl p-8 mb-12">
          <h2 className="text-lg font-semibold mb-4 text-red-400">Required CSV Format</h2>
          <p className="text-gray-300 mb-4">Your CSV file must contain these columns:</p>
          <div className="grid grid-cols-2 gap-3 text-sm text-gray-400">
            <div>• Billing Phone</div>
            <div>• CallAudio (URL)</div>
          ...
          <p className="text-gray-400 text-sm mt-4 leading-relaxed">
            Note: Post-purchase calls (detected in first 20s) will be automatically filtered out.
          </p>
        </div>
          ...
          <div className="space-y-8">
            ...
            >
              ...
              <div className="flex flex-col items-center gap-4">
                <div className="p-4 bg-red-900/20 rounded-xl border border-red-600/30">
                ...
                <button
                  ...
                  className="mt-4 px-6 py-2 bg-red-600/20 border border-red-500/40 rounded-lg text-red-400 hover:bg-red-600/30 transition text-sm font-medium"
                >
                  Select File
                </button>
              </div>
                ...
                >
                  {isProcessing ? (
                    <div className="flex items-center justify-center gap-2">
                      ...
                      Processing... (Detecting Pre/Post Purchase & Analyzing)
                    </div>
                  ) : (
                    'Start Processing'
                  )}
        ...
        {uploadResult && (
          <div className={`bg-[#0f0f14] border rounded-2xl p-8 ${
              ...
              {uploadResult.status === 'failed' ? (
                ...
                <>
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-500 mt-1">{uploadResult.message}</p>
                  </div>
                </>
              )}
            </div>
            ...
            {uploadResult.status !== 'failed' && (
              <div className="grid grid-cols-4 gap-4 mb-8">
                ...
                <div className="bg-green-900/20 border border-green-500/30 rounded-lg p-4">
                  <p className="text-xs uppercase tracking-wider text-green-400 mb-2">Analyzed (Pre)</p>
                  <p className="text-2xl font-bold text-green-400">{uploadResult.successful || 0}</p>
                </div>
                <div className="bg-gray-700/20 border border-gray-600/30 rounded-lg p-4">
                   <p className="text-xs uppercase tracking-wider text-gray-400 mb-2">Filtered (Post)</p>
                   <p className="text-2xl font-bold text-gray-300">{uploadResult.filtered_out || 0}</p>
                </div>
              </div>
            )}
            ...
            {uploadResult.errors && uploadResult.errors.length > 0 && (
              <div className="mb-8">
                <h3 className="text-sm font-semibold text-gray-300 mb-4">Error Summary</h3>
            ...
            <div className="flex gap-4">
              ...
              <button
                onClick={handleBackToReports}
                className="flex-1 py-3 rounded-lg bg-red-600 text-white hover:bg-red-700 transition font-semibold"
              >
                View ABC Call Reports
              </button>
            </div>
          </div>
        )}
